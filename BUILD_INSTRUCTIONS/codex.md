...
3. Verify baseline: `atb validate ../fixtures` and `atb selftest` must pass before you change logic.
4. Implement the Work Breakdown in order; prioritize WB3 (replay), WB4 (scorers), WB5 (runner).
5. Write a pytest for each scorer as you implement it. CI runs `atb validate`, `atb selftest`, `pytest -q`.
