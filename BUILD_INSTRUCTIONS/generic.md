...
You have been handed the **AgentTool-Bench (ATB)** package. Your job: complete the reference harness so `atb run` produces a scored report against real-world failure fixtures.
...
pip install -e .[dev]
atb validate ../fixtures      # must pass on provided fixtures (WB1)
atb selftest                  # must produce a grade via the echo adapter
```
...
## Step 3 — Prove it
- `pytest -q` green.
- `atb run --adapter examples/echo_adapter.py --suite core` is deterministic.
- Add at least 6 more fixtures from `fixtures/CATALOG.md` rows marked "no (WB8)", each with real provenance.

## Hard rules
- Never turn ATB into a generator.
