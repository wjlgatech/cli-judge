...
You are building **CLI-Judge**, a reality-grounded benchmark + verification harness for agent-native CLIs / MCP servers / software harnesses. This file is your single source of truth for *what to build and in what order*. Read `SPEC.md` and `RUBRIC.md` for the *what it measures*, then return here for the *work*.

## Prime directives

1. **CLI-Judge measures tools; it does not generate them.** Reject any task that drifts toward building a code generator.
...
```
adapter (per tool-under-test)  ->  CLI-Judge runner  ->  replay engine  ->  scorers  ->  report
        |                              |               |               |            |
   normalizes a tool          loads suite +     matches tool I/O    5 dimension   JSON + md
   into CLI-Judge's call ABI        fixtures          to fixtures         scorers       scorecard
```

- **Adapter**: a small Python file exposing `invoke(call) -> Result`. It is how CLI-Judge talks to *any* tool (a Go pp-cli binary, a Python cli-anything harness, an MCP server, a mock). Adapters are the only tool-specific code.
...
- `cli-judge validate ./fixtures` exits 0 on all provided fixtures.
- `cli-judge run --adapter examples/echo_adapter.py --suite core` produces a `report.json` and `scorecard.md` and a printed grade, deterministically (same input -> same output).
